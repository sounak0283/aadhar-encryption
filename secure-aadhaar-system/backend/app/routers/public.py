...
from app.services import audit_service, submission_service, user_session
...
async def submit_aadhaar(
    ...
    if not is_valid_aadhaar_number(payload.aadhaar_number):
        await audit_service.record_submit(session.username, "invalid_aadhaar_number")
        raise HTTPException(status_code=400, detail="invalid Aadhaar number")

    reason = freshness_error(payload.ts)
    if reason is not None:
        await audit_service.record_submit(session.username, reason)
    ...
    try:
        ...
        await audit_service.record_submit(session.username, "no_active_admins")
        raise HTTPException(status_code=503, detail="no admin is registered yet — submissions are not accepted")

    await audit_service.record_submit(session.username, "success", container_id=reference_id)
